...
import { Bell, BookOpen, ExternalLink, Folder, Image, LogOut, RotateCcw, User } from 'lucide-react'
...
export default function CreateArticlePage({ onArticles, onWebsite, onLogout, onSave }: CreateArticlePageProps) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>, status: 'draft' | 'published') => {
    ...
    try {
      ...
      setMessage(status === 'draft' ? 'Article saved as draft.' : 'Article saved and published.')
      onArticles()
    } catch (error) {
      ...
      <aside className="admin-shell__sidebar" aria-label="Admin navigation">
        <div>
          ...
          <nav className="admin-shell__nav">
            <button type="button" className="admin-shell__nav-item admin-shell__nav-item--active" onClick={onArticles}>
              <BookOpen size={20} strokeWidth={1.6} /><span>Article management</span>
            </button>
      ...
      <main className="create-article">
        <header className="create-article__header">
          ...
          <div className="create-article__header-actions">
            <button disabled={saving} type="submit" form="create-article-form" name="intent" value="draft" className="create-article__draft">Save as draft</button>
            <button disabled={saving} type="submit" form="create-article-form" name="intent" value="published" className="create-article__publish">{saving ? 'Saving...' : 'Save and publish'}</button>
          </div>
        </header>
        ...
        >
          <section className="create-article__thumbnail-section">
            ...
            <div className="create-article__thumbnail-row">
              <div className="create-article__thumbnail">
                {thumbnail ? <img src={thumbnail} alt="Thumbnail preview" /> : <Image size={34} strokeWidth={1.3} aria-hidden="true" />}
              </div>
          ...
          <label className="create-article__field create-article__field--short create-article__field--disabled">
            ...
            <input value="Thompson P." disabled />
          </label>
          ...
          <label className="create-article__field">
            ...
            <textarea name="introduction" placeholder="Introduction" maxLength={120} rows={5} required />
          </label>

          <label className="create-article__field">
            <span>Content</span>
            <textarea name="content" placeholder="Content" className="create-article__content-input" required />
          </label>
